...
Q.fcall(function() { //第一步：监听浏览器自动刷新端口
    ...
    .then(function() { //第二步：配置注入插件
        var plugins = [],
            deferred = Q.defer();

        //livereload中间件
        var livereload = function(opts) {
            return require('connect-livereload')({
                port: port
            });
        };
        //openBrowser中间件
        var openBrowser = function(opts) {
            opts.isCluster = false;
            opts.openBrowser = function(serverURL) {
                if (autoOpenBrowser) {
        ...
        //TODO:livereload这个自动刷新插件影响nodejs代理文件下载功能，目前文件下载尚未使用平台文件服务，不可集群，为了解决多层代理问题，
        //文件请求的host暂时由前端通过请求头（client）转给后端,下一个版本（2015年11月）集成平台文件服务之后，这个插件再启用。
        // plugins.push(livereload);
        plugins.push(openBrowser);

        deferred.resolve(plugins);
        return deferred.promise;
    })
    .then(function(plugins) { //第三步：启动Web服务器
        var deferred = Q.defer();
        server.start(plugins);
        deferred.resolve(true);
        return deferred.promise;
    })
    .then(function() { //第四步：监控文件修改结果，执行代码审查，然后重启服务器或者刷新浏览器
